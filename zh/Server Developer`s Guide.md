...
> \[参考\]\
> 结算 1次可消费 1次，未消费结算的结算被当作是在IAP上未支付项目。\
> 客户端可批量查询未消费的结算项。
...
虽已完成结算，但是未消费(consume)的结算明细可用SERVER
...
\[Header\]
  ...
  Property name   Value     Description
  --------------- --------- ------------------------------------------------------
  isSuccessful    Boolean   确认查询是否正常，成功邀请API时为true，失败时为false
...
\[Result\]
  ...
  marketId         String   ITEM 的市场划分Google : "GG" 苹果 : "AS" ONE Store : "TS"
