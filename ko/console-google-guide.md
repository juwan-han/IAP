...
- 아래 가이드 내용대로 설정 완료 후 스크린샷과 같은 형태로 Google Play Console에서 연결된 프로젝트의 상태가 나타나지 않는다면, Google Cloud와의 연결 부분을 우선 확인하세요.
...
- NHN Cloud IAP는 두가지 인증 모델을 제공하며, 두 인증 모델들은 각기 인증을 위해 각기 다른 특화 정보들이 필요합니다.
- 모델별 특화 정보 외 공통적으로 필요한 정보도 Google 연동 및 결제 확인을 위해 필요하므로, **공통 입력 정보도 함께 확인**을 부탁 드립니다.
...
- 서비스 계정은 최고 관리자에의해 최고 관리자에 준하는 범위의 권한을 위임받을 수도 있고, 최고 관리자가 권한을 가진 특정 앱만으로 한정된 범위의 권한을 위임받을 수도 있습니다.
    - 위임 권한 범위에 대한 설정 전략은 고객이의도한 특성에 맞게 설정하시면 됩니다.
...
4. 서비스 계정으로의 권한 위임
    ...
    - 범위는 고객의의도에 맞게 설정하되, 권한의 종류 중 `앱 정보 보기 (읽기 전용)` , `재무 데이터 보기`, `주문 및 구독 관리`의 권한은 필수로 위임 설정해야 합니다.
